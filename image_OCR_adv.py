...
import pylab
from keras import backend as K, optimizers as ko
...
class TextImageGenerator(keras.callbacks.Callback):
    ...
    def get_batch(self, index, size, train):
        ...
        outputs = {'ctc': np.zeros([size]),
                   'province': Y_data}  # dummy data for dummy loss function
...
class VizCallback(keras.callbacks.Callback):
    ...
    def show_edit_distance(self, num):
        ...
        while num_left > 0:
            ...
            decoded_res, province = decode_batch(self.test_func,
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        word_batch = next(self.text_img_gen)[0]
        res, province = decode_batch(self.test_func,
                                     word_batch['the_input'][0:self.num_display_words])
        cols = 2
        for i in range(self.num_display_words):
            pylab.subplot(self.num_display_words // cols, cols, i + 1)
            the_input = word_batch['the_input'][i, :, :, :]
            pylab.imshow(np.rollaxis(the_input, 1))
            pylab.xlabel(
                'Truth = \'%s\'\nDecoded = \'%s\'\nProv = %s' %
                (word_batch['source_str'][i], res[i], province[i]), fontproperties=prop)
        fig = pylab.gcf()
...
def train(run_name, start_epoch, stop_epoch, img_w, img_h=120):
    # Input Parameters
    ...
    sgd = ko.SGD(lr=0.01, decay=1e-6, momentum=0.9, clipnorm=5)

    model = Model(inputs=[input_data, labels, input_length, label_length],
                  outputs=[loss_out, classifier])

    # the loss calc occurs elsewhere, so use a dummy lambda func for the loss
    model.compile(loss={'ctc': lambda y_true, y_pred: y_pred, 'province': 'categorical_crossentropy'}, optimizer=sgd)
    if start_epoch > 0:
        weight_file = os.path.join(
            OUTPUT_DIR,
            os.path.join(run_name, 'weights%02d.h5' % (start_epoch - 1)))
        model.load_weights(weight_file)
    # model.load_weights('image_ocr_LP/2019:04:06:01:57:15/weights10.h5')
    # captures output of softmax so we can decode the output during visualization
    test_func = K.function([input_data], [y_pred, classifier])

    viz_cb = VizCallback(run_name, test_func, img_gen.next_val(), num_display_words=8)

    # tensorboard = keras.callbacks.TensorBoard(os.path.join(OUTPUT_DIR, run_name), write_graph=True)
