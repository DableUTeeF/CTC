...
def get_model(absolute_max_string_len, img_w, img_h=40):
    ...
    model = Model(inputs=[input_data, labels, input_length, label_length],
                  outputs=loss_out)
    model.load_weights('image_ocr_LP/2019:03:21:16:08:25/weights22.h5')
...
if __name__ == '__main__':
    w, h = 108, 40
    model, test_func = get_model(7, w, h)
    image = Image.open('plate.jpeg').convert('L').resize((w, h))
    im = np.array(image).astype('float32') / 255.
    im = np.expand_dims(im.T, 2)
