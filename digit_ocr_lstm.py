...
class DigitImageGenerator(keras.callbacks.Callback):
    ...
    def build_word_list(self, num_words, max_string_len=None, mono_fraction=0.5):
        ...
        for i in range(16000):
            length = np.random.randint(3, max_string_len)
            tmp_string = ''
            for l in range(length):
                tmp_string += str(np.random.randint(0, 9))
    ...
    def on_epoch_begin(self, epoch, logs={}):
        ...
        if 3 <= epoch < 6:
            # if self.max_string_len < 8:
            #     self.build_word_list(16000, 8, 0.5)

            self.paint_func = lambda text: paint_text(
                text, self.img_w, self.img_h,
                rotate=False, ud=True, multi_fonts=True)
        elif 6 <= epoch < 9:
            # if self.max_string_len < 9:
            #     self.build_word_list(16000, 9, 0.5)
            self.paint_func = lambda text: paint_text(
                text, self.img_w, self.img_h,
                rotate=False, ud=True, multi_fonts=True)
        elif epoch >= 9:
            # if self.max_string_len < 10:
            #     self.build_word_list(16000, 10, 0.5)
            self.paint_func = lambda text: paint_text(
                text, self.img_w, self.img_h,
...
def train(run_name, start_epoch, stop_epoch, img_w):
    ...
    inner = Conv2D(conv_filters, kernel_size, strides=2, padding='same',
                   activation=act, kernel_initializer='he_normal',
                   name='conv1')(input_data)
    # inner = MaxPooling2D(pool_size=(pool_size, pool_size), name='max1')(inner)
    inner = Conv2D(conv_filters, kernel_size, strides=1, padding='same',
                   activation=act, kernel_initializer='he_normal',
                   name='conv2')(inner)
    inner = Conv2D(conv_filters, kernel_size, strides=2, padding='same',
                   activation=act, kernel_initializer='he_normal',
    ...
    inner = kl.Permute((3, 2, 1))(inner)
    # cuts down input size going into RNN:
    inner = kl.TimeDistributed(kl.LSTM(28, return_sequences=True))(inner)
    inner = kl.TimeDistributed(kl.LSTM(rnn_size, return_sequences=True))(inner)
    inner = kl.Permute((3, 2, 1))(inner)
    inner = Reshape((rnn_size, 16 * 16))(inner)
    ...
    tensorboard = keras.callbacks.TensorBoard(os.path.join(OUTPUT_DIR, run_name, 'log'), write_graph=True)
    model.fit_generator(
