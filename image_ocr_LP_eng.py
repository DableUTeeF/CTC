...
class VizCallback(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        for i in range(self.num_display_words):
            ...
            # the_output = paint_text(res[i], h=32, w=128)
            # pylab.subplot(self.num_display_words // cols, cols, i + 2)
            # pylab.imshow(the_output[0])
            pylab.xlabel(
                'Truth = \'%s\'\nDecoded = \'%s\'' %
                (word_batch['source_str'][i], res[i]), fontproperties=prop)
        fig = pylab.gcf()
...
def train(run_name, start_epoch, stop_epoch, img_w, img_h=40):
    # Input Parameters
